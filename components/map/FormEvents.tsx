...
export default function FormEvents() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        server: `Error al ${selectedEvent ? "actualizar" : "crear"} el evento. Inténtalo de nuevo.`,
      });
    } finally {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      setIsGeocoding(true);
      try {
        const bias = mapCenter
          ? { lng: mapCenter[0], lat: mapCenter[1] }
          : { lng: -74.7813, lat: 10.9685 };
        const results = await hybridGeocode(
          addressInput,
          bias,
  ...
  useEffect(() => {
    if (selectedEvent && isEventFormOpen) {
      ...
      const loc = {
        lat: parseFloat(String(selectedEvent.lat)),
        lng: parseFloat(String(selectedEvent.lng)),
      };
  ...
  return (
    <>
      ...
      <div className="fixed sm:absolute bottom-0 sm:bottom-auto sm:top-20 right-0 sm:right-8 z-20 w-full sm:w-[380px]">
        <Card
          size="default"
          className="w-full h-auto max-h-[85vh] sm:max-h-[85vh] flex flex-col overflow-hidden shadow-2xl border-muted/50 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60 rounded-t-3xl sm:rounded-xl"
        >
          {/* Header Image Area */}
          <Field className="relative">
            <label
              htmlFor="picture"
              className="w-full h-36 bg-muted/40 flex flex-col items-center justify-center relative group cursor-pointer border-b hover:bg-muted/60 transition-colors"
            >
              <Input
                id="picture"
                type="file"
          ...
          <CardContent className="overflow-y-auto pr-3 space-y-4 flex-1 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:bg-muted [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
            <form className="flex flex-col gap-4">
              <div className="grid gap-1.5">
                <Label
                  htmlFor="title"
                  className="flex items-center gap-1.5 text-foreground/80 font-semibold text-xs uppercase tracking-wider"
                >
              ...
              <div className="grid gap-1.5">
                <Label
                  htmlFor="description"
                  className="text-foreground/80 font-semibold text-xs uppercase tracking-wider"
                >
                  Descripción
                </Label>
                <textarea
              ...
              <div className="grid gap-1.5">
                <Label
                  ...
                  className="flex items-center gap-1.5 text-foreground/80 font-semibold text-xs uppercase tracking-wider"
                >
                  <MapPin className="w-3.5 h-3.5 text-primary" /> Lugar /
                  Ubicación
                </Label>
                <div className="relative group/input">
                  <Input
                    ...
                    onFocus={() => {
                      ...
                      if (addressInput && suggestions.length === 0)
                ...
                {errors.location && (
                  <p className="text-[10px] font-medium text-destructive mt-1 flex items-center gap-1 leading-tight">
                    <X className="w-3 h-3" /> {errors.location}
                  </p>
                )}
                {suggestions.length > 0 && (
                  <p className="text-[10px] font-medium text-primary mt-1 animate-pulse flex items-center gap-1">
              ...
              <div className="grid gap-1.5 pb-2">
                <Label className="text-foreground/80 font-semibold text-xs uppercase tracking-wider">
                  Map Icon
                </Label>
